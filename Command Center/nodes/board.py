...
def printPiece(head, lB, rB, gameDisplay):
    red = (255,0,0)
    green = (0,255,0)
    blue = (0,0,255)
    if head[1] < lB[1]:
    ...
    #pygame.draw.circle(gameDisplay, green, head, int(5), int(0))
...
def printPieces(pieceLoc, gameDisplay): #pieceLoc is list containing a list for each piece's location points
    for piece in pieceLoc:
        printPiece(piece[0],piece[1],piece[2], gameDisplay)
    pygame.display.update()
...
def moveWheels(lV, rV, pieceLoc, p1, color, gameDisplay):
    ...
    if color == 'white':
        for l, r in zip(lV,rV):
            printBoard(gameDisplay)
            printPieces(pieceLoc, gameDisplay)
            # timeStep(1)
